...
def int0(x) -> int:
    """None/NaN/空文字を安全に 0 へ落とす int キャスト。"""
    v = safe_int(x)
...
def float_default(x, default: float) -> float:
    """None/NaN を default に落とす float キャスト。"""
    v = safe_float(x)
    return float(v) if v is not None else float(default)


def float_default(x, default: float) -> float:
    """None/NaN の場合は default を返す float キャスト。"""
    v = safe_float(x)
    return float(v) if v is not None else float(default)
...
.pill.ng{ background: rgba(225,29,72,0.10); color: var(--danger); }
...
<div class="record-card">
  ...
  <div class="section-sub"><span style='font-size:12px;color:rgba(15,23,42,0.68);'>{info_line}</span><br>{badge} / {temp_line} / {meal_line} / 巡視:{patrol_count}回</div>
    ...
    # Tabs
    tab_record, tab_status, tab_handover = st.tabs(["✍️ 記録入力・一覧", "📊 実施状況", "🗒️ 申し送り（連絡帳）"])
    ...
    with tab_record:
        ...
        # バイタル初期値（未入力による None/計算エラー防止）
        std = {
            "temp_am": 36.0, "bp_sys_am": 120, "bp_dia_am": 80, "pulse_am": 70, "spo2_am": 98,
            "temp_pm": 36.0, "bp_sys_pm": 120, "bp_dia_pm": 80, "pulse_pm": 70, "spo2_pm": 98,
        }
        def dv(key, as_type):
            v = latest_v.get(key)
        ...
        with st.container():
            st.markdown('<div class="record-card">', unsafe_allow_html=True)
            st.markdown('<div class="section-title">④ 服薬（OK）</div>', unsafe_allow_html=True)

            a, b, c, d = st.columns(4)
            with a:
                med_m = st.checkbox("朝OK", value=False, key=add_key("add_med_m"))
            with b:
                med_n = st.checkbox("昼OK", value=False, key=add_key("add_med_n"))
            with c:
                med_e = st.checkbox("夕OK", value=False, key=add_key("add_med_e"))
            with d:
                med_b = st.checkbox("寝る前OK", value=False, key=add_key("add_med_b"))
        ...
        if save_clicked:
            if recorder_name.strip() == "":
                ...
                if can_save:
                    ...
                    def n_real_default(x, default: float) -> float:
                        return float_default(x, default)

                    def n_int_default(x, default: int) -> int:
                        v = safe_int(x)
                        return int(v) if v is not None else int(default)
    
                    wakeup_flag = 1 if str(add_scene) == "起床" else 0
    
                    # 未測定ONなら全部None / それ以外は「空欄(=None/NaN)なら初期値」を強制
                    am_temp_v = None if am_unmeasured else n_real_default(am_temp, 36.0)
                    am_sys_v = None if am_unmeasured else n_int_default(am_sys, 120)
                    am_dia_v = None if am_unmeasured else n_int_default(am_dia, 80)
                    am_pulse_v = None if am_unmeasured else n_int_default(am_pulse, 70)
                    am_spo2_v = None if am_unmeasured else n_int_default(am_spo2, 98)

                    pm_temp_v = None if pm_unmeasured else n_real_default(pm_temp, 36.0)
                    pm_sys_v = None if pm_unmeasured else n_int_default(pm_sys, 120)
                    pm_dia_v = None if pm_unmeasured else n_int_default(pm_dia, 80)
                    pm_pulse_v = None if pm_unmeasured else n_int_default(pm_pulse, 70)
                    pm_spo2_v = None if pm_unmeasured else n_int_default(pm_spo2, 98)
        ...
        if recs.empty:
            ...
            for _, r in recs.iterrows():
                ...
                # None/NaN を考慮して安全に判定
                meds_any = (
                    int0(r.get("med_morning")) == 1
                    or int0(r.get("med_noon")) == 1
                    or int0(r.get("med_evening")) == 1
                    or int0(r.get("med_bed")) == 1
                )
                if meds_any:
...
{patrol_inline}
    ...
    with tab_status:
        st.markdown("### 📊 本日の実施状況")
        st.caption("全利用者の当日状況（バイタル体温／食事量／服薬OK）を一覧で確認できます。")
        ...
        def pill(ok_flag: bool, ok_text: str, ng_text: str = "ー") -> str:
            if ok_flag:
        ...
        for _, rr in residents_df.iterrows():
            ...
            vit_am_done = bool(int0(s.get("vit_am_done")) == 1) if s is not None else False
            vit_pm_done = bool(int0(s.get("vit_pm_done")) == 1) if s is not None else False
            temp_am = safe_float(s.get("temp_am")) if s is not None else None
            temp_pm = safe_float(s.get("temp_pm")) if s is not None else None

            meal_bf_done = bool(int0(s.get("meal_bf_done")) == 1) if s is not None else False
            meal_lu_done = bool(int0(s.get("meal_lu_done")) == 1) if s is not None else False
            meal_di_done = bool(int0(s.get("meal_di_done")) == 1) if s is not None else False
            meal_bf_score = int0(s.get("meal_bf_score")) if s is not None else 0
            meal_lu_score = int0(s.get("meal_lu_score")) if s is not None else 0
            meal_di_score = int0(s.get("meal_di_score")) if s is not None else 0

            med_m = bool(int0(s.get("med_morning")) == 1) if s is not None else False
            med_n = bool(int0(s.get("med_noon")) == 1) if s is not None else False
            med_e = bool(int0(s.get("med_evening")) == 1) if s is not None else False
            med_b = bool(int0(s.get("med_bed")) == 1) if s is not None else False
...
<div class='record-card'>
  ...
  <hr>
  <div style='font-weight:900;margin-bottom:6px;'>バイタル（体温）</div>
    ...
    with tab_handover:
        ...
        if ho.empty:
            ...
            for _, h in ho.iterrows():
                ...
                if st.button("🗑️ 削除", key=f"del_ho_{hid}"):
                    st.session_state[confirm_key] = True

                if st.session_state.get(confirm_key, False):
                    st.warning("本当に削除しますか？", icon="⚠️")
                    c1, c2 = st.columns(2)
                    with c1:
                        if st.button("削除する", key=f"del_ho_yes_{hid}"):
                            soft_delete_handover(conn, hid)
                            st.session_state[confirm_key] = False
                            st.success("✅ 削除しました")
                            st.rerun()
                    with c2:
                        if st.button("キャンセル", key=f"del_ho_no_{hid}"):
                            st.session_state[confirm_key] = False
                            st.rerun()
